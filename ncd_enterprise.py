...
class NCDEnterprise:
    ...
    def parse(self, xbee_packet):
        if not isinstance(xbee_packet, ATCommResponsePacket):
            ...
            # TODO: find out what type was intended to be used for
            # type = self.payload_type[str(data[0])]
            if (callable(getattr(self, packet_type))):
                getattr(self, packet_type)(
                    data[1:], xbee_packet.x64bit_source_addr
                )
        else:
            print(xbee_packet)
    ...
    def buffer_mems(self, payload, source_address):
        ...
        # If there is an ongoing error, process it. This is primarily to
        # reduce multiple instances of data corruption errors for the
        # same dataset from the sensor
        ...
        if self.mems_buffer.get(source_address).get('timeout_exception_start'):
            ...
            elapsed_millis = (
                ...
                self.mems_buffer.get(source_address).get(
                    'timeout_exception_start')
            )
            if elapsed_millis > 500 or payload[1] == 12:
                self.mems_buffer[source_address][payload[1]] = payload[5:]
                # set error
                self.parse_error_callback(self.mems_buffer.get(source_address))
                # clear buffer
                self.mems_buffer[source_address] = {}
                # if the current payload is not a new dataset, do not process
                # further
                if payload[1] != 1:
                    return

        if payload[1] not in self.mems_buffer.get(source_address):
            if len(self.mems_buffer.get(source_address)) == (
                    int(payload[1]) - 1):
                # packet has been shifted left one, therefore data will start
                # at byte 5.
                self.mems_buffer[source_address][payload[1]] = payload[5:]
            else:
                self.mems_buffer[source_address][payload[1]] = payload[5:]
                self.mems_buffer[source_address]['timeout_exception_start'] = (
                    self.get_current_millis())
                return
        else:
        ...
        if (len(self.mems_buffer.get(source_address)) == 12):
            # packet from intercept has first byte trimmed. Shift expected
            # position left. i.e. node_id is byte 0 instead of documented one.
            self.parse_mems(
                self.mems_buffer.get(source_address),
                source_address,
                payload)
            self.mems_buffer[source_address] = {}

    # TODO configuration commands put on hiatus. Need to import struct lib to
    # ensure packet compatibility. AKA sleep_time needs to be 3 bytes, if a
    # single int is passed it won't be. extensive testing needed.
    # def sensor_set_node_id_sleep(
    #    self, target_address, node_id, sleep_time, log = True):
    #     node_id = bytearray(node_id)
    #     sleep_time = bytearray(node_id)
    #     self.send_data_to_address(
    #         target_address,
    #         bytearray.fromhex('f702000000')+node_id+sleep_time)
    ...
    def parse_mems(self, mems_dict, source_address, last_payload):
        ...
        for index, packet in enumerate(mems_dict):
            ...
            # TODO: find out what packet_array intended use is
            # packet_array = {}
            for reading in range(1, readings+1):
                if packet == 12 and reading >= 22:
                    break
                reading_array[((index*readings)+reading)] = (
                    packet_data[((reading-1) * bytes_in_single):(reading-1) *
                                bytes_in_single + bytes_in_single])
        ...
        reading_array['temperature'] = msbLsb(
            last_payload[-6], last_payload[-5])
        battery_reading = msbLsb(last_payload[-3], last_payload[-2]) * 0.00322
        parsed = {
            'nodeId': last_payload[0],
            'odr': last_payload[4],
            'firmware': last_payload[-4],
            'battery': battery_reading,
            'battery_percent': str((battery_reading - 1.3)/2.03*100) + "%",
            'counter': 'NA',
            'sensor_type_id': 40,
            'source_address': str(source_address),
    ...
    def sensor_data(self, payload, source_address):
        battery_reading = msbLsb(payload[2], payload[3]) * 0.00322
        parsed = {
            'nodeId': payload[0],
            'firmware': payload[1],
            'battery': battery_reading,
        ...
        try:
            if (parsed['sensor_type_id'] == 80):
                ...
                parsed['sensor_type_string'] = self.sensor_types[
                    str(parsed['sensor_type_id'])]['name']
                parsed['sensor_data'] = self.sensor_types[
                    str(parsed['sensor_type_id'])]['parse'](payload[8:])

        except:
            # TODO: find out the specific exception to use here
            # it is not correct ot use bare except
...
def sensor_types():
    types = {
        ...
        '10': {
            'name': 'Light',
            'parse': lambda d: {'lux': reduce(msbLsb, d[0:3])}
        },
        # untested
        '12': {
        ...
        '16': {
            ...
            'parse': lambda d: (
                lambda adc1=reduce(msbLsb, d[0:2]), adc2=reduce(msbLsb, d[0:2]): {
                    'adc1': adc1,
                    'adc2': adc2,
                    'voltage1': adc1 * 0.00322265625,
                    'voltage2': adc2 * 0.00322265625,
                    'percentage': 100 if (adc1 > 870) else round(adc2 / 870 * 100)
                }
            )()
        },
        # untested
        ...
        '50': {
            ...
            'parse': lambda d: {
                ...
                'thermocouple_temperature': signInt(
                    reduce(msbLsb, d[29:33]), 32) / 100,
        ...
        '80': {
            'name': 'Predictive Maintenance Sensor',
            'parse': lambda d: (
                "This is a more complex sensor and so is parsed using the " +
                "defined function 'type80'")
        },
    }
    return types
